...
pred eventBasedGatewayHappens(ps: ProcessSnapshot, ebg: EventBasedGateway, osf: SequenceFlow) {
    -- Preconditions
	one t: ps.tokens {
		t.pos = ebg
		osf in ebg.outgoingSequenceFlows
		-- post conditions
		some newToken : Token'-Token {
			Token' = Token + newToken - t
...
pred exitSubProcess(ps: ProcessSnapshot, sp: SubProcess, spEndEvent: EndEvent, incomingFlow: SequenceFlow, outgoingFlow: SequenceFlow) {
    -- Preconditions
    one t: ps.tokens {
        incomingFlow.target = spEndEvent               
        t.pos = incomingFlow                         
        outgoingFlow in sp.outgoingSequenceFlows       
...
pred trans [] {
	...
	or
    	...
    	or
    	(some ps: ProcessSnapshot, ebg: EventBasedGateway, osf: SequenceFlow | eventBasedGatewayHappens[ps, ebg, osf])
    	or
...
one sig eventBasedGateway1 extends EventBasedGateway {}
one sig intermediateCatchEvent1 extends IntermediateCatchEvent {}
one sig intermediateCatchEvent2 extends IntermediateCatchEvent {}
one sig endEvent1 extends EndEvent {}
one sig endEvent2 extends EndEvent {}

pred init {
    #intermediateCatchEvent2.outgoingSequenceFlows = 1
    intermediateCatchEvent2.outgoingSequenceFlows.target = task1
    #intermediateCatchEvent1.outgoingSequenceFlows = 1
    intermediateCatchEvent1.outgoingSequenceFlows.target = task2
    #eventBasedGateway1.outgoingSequenceFlows = 2
    eventBasedGateway1.outgoingSequenceFlows.target = intermediateCatchEvent1 + intermediateCatchEvent2
    #task1.outgoingSequenceFlows = 1
    task1.outgoingSequenceFlows.target = endEvent2
    #task2.outgoingSequenceFlows = 1
    task2.outgoingSequenceFlows.target = endEvent1
    #startEvent1.outgoingSequenceFlows = 1
    startEvent1.outgoingSequenceFlows.target = eventBasedGateway1
    #intermediateCatchEvent2.incomingSequenceFlows = 1
    intermediateCatchEvent2.incomingSequenceFlows.source = eventBasedGateway1
    #intermediateCatchEvent1.incomingSequenceFlows = 1
    intermediateCatchEvent1.incomingSequenceFlows.source = eventBasedGateway1
    #eventBasedGateway1.incomingSequenceFlows = 1
    eventBasedGateway1.incomingSequenceFlows.source = startEvent1
    #task1.incomingSequenceFlows = 1
    task1.incomingSequenceFlows.source = intermediateCatchEvent2
    #task2.incomingSequenceFlows = 1
    task2.incomingSequenceFlows.source = intermediateCatchEvent1
    #endEvent2.incomingSequenceFlows = 1
    endEvent2.incomingSequenceFlows.source = task1
    #endEvent1.incomingSequenceFlows = 1
    endEvent1.incomingSequenceFlows.source = task2
...
run System for 1 StartEvent, 2 Task, 0 ExGate, 0 PaGate, 1 EventBasedGateway, 2 IntermediateCatchEvent, 0 IntermediateThrowEvent, 2 EndEvent, 0 TerminateEndEvent, 0 SubProcess, 7 SequenceFlow, 1 Process, 2 Token, 1 ProcessSnapshot, 0 Event, 12 steps
