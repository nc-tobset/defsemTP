...
sig SpEndEvent extends FlowNode {
}
...
pred exitSubProcess(ps: ProcessSnapshot, sp: SubProcess, spEndEvent: SpEndEvent, incomingFlow: SequenceFlow, outgoingFlow: SequenceFlow) {
    -- Preconditions
    one t: ps.tokens {
...
pred trans [] {
	...
	or
    	...
    	or
	(some ps: ProcessSnapshot, sp: SubProcess, spEndEvent: SpEndEvent, incomingFlow: SequenceFlow, outgoingFlow: SequenceFlow | exitSubProcess[ps, sp, spEndEvent, incomingFlow, outgoingFlow])
    	or
	doNothing
}
...
one sig parallelGatewayOpening1 extends PaGate {}
one sig parallelGatewayClosing1 extends PaGate {}
one sig endEvent1 extends SpEndEvent {}
one sig endEvent2 extends EndEvent {}
one sig subProcess1 extends SubProcess {}

pred init {
    #subProcess1.outgoingSequenceFlows = 1
    subProcess1.outgoingSequenceFlows.target = task4
    #parallelGatewayClosing1.outgoingSequenceFlows = 1
    parallelGatewayClosing1.outgoingSequenceFlows.target = endEvent1
    #parallelGatewayOpening1.outgoingSequenceFlows = 2
    parallelGatewayOpening1.outgoingSequenceFlows.target = task2 + task3
    #task1.outgoingSequenceFlows = 1
    task1.outgoingSequenceFlows.target = subProcess1
    #task2.outgoingSequenceFlows = 1
    task2.outgoingSequenceFlows.target = parallelGatewayClosing1
    #startEvent2.outgoingSequenceFlows = 1
    startEvent2.outgoingSequenceFlows.target = parallelGatewayOpening1
    #task3.outgoingSequenceFlows = 1
    task3.outgoingSequenceFlows.target = parallelGatewayClosing1
    #startEvent1.outgoingSequenceFlows = 1
    startEvent1.outgoingSequenceFlows.target = task1
    #task4.outgoingSequenceFlows = 1
    task4.outgoingSequenceFlows.target = endEvent2
    #subProcess1.incomingSequenceFlows = 1
    subProcess1.incomingSequenceFlows.source = task1
    #parallelGatewayClosing1.incomingSequenceFlows = 2
    parallelGatewayClosing1.incomingSequenceFlows.source = task2 + task3
    #parallelGatewayOpening1.incomingSequenceFlows = 1
    parallelGatewayOpening1.incomingSequenceFlows.source = startEvent2
    #task1.incomingSequenceFlows = 1
    task1.incomingSequenceFlows.source = startEvent1
    #task2.incomingSequenceFlows = 1
    task2.incomingSequenceFlows.source = parallelGatewayOpening1
    #endEvent2.incomingSequenceFlows = 1
    endEvent2.incomingSequenceFlows.source = task4
    #task3.incomingSequenceFlows = 1
    task3.incomingSequenceFlows.source = parallelGatewayOpening1
    #task4.incomingSequenceFlows = 1
    task4.incomingSequenceFlows.source = subProcess1
    #endEvent1.incomingSequenceFlows = 1
    endEvent1.incomingSequenceFlows.source = parallelGatewayClosing1
    #startEvent1.incomingSequenceFlows = 0
    #startEvent2.incomingSequenceFlows = 0
    #endEvent1.outgoingSequenceFlows = 0
    #endEvent2.outgoingSequenceFlows = 0
    one t1: ProcessSnapshot.tokens | t1.pos = startEvent1
}

run System for 2 StartEvent, 4 Task, 0 ExGate, 2 PaGate, 0 EventBasedGateway, 0 IntermediateCatchEvent, 0 IntermediateThrowEvent, 1 EndEvent, 1 SpEndEvent, 0 TerminateEndEvent, 1 SubProcess, 10 SequenceFlow, 1 Process, 3 Token, 1 ProcessSnapshot, 1 Event, 23 steps
